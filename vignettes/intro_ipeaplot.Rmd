...
```{r eval=TRUE, fig.width=8, fig.height=4, message=FALSE, warning=FALSE, out.width = "100%"}
fig_raw <- ggplot() +
...
Nesse outro gráfico, a gente visualiza a dispersão da expectativa de vida dos municípios em cada região do país no ano de 2010.
...
ggplot(data = df2) +
  geom_boxplot(aes(x = reorder(regiao, espvida), y=espvida, weight = pop, color = regiao)) +
  scale_color_ipea(discrete = T, palette = 'Blue') +
  labs(title = 'Proporção da população com coleta de lixo', color='Região') +
  ylab('Proporção em %') +
  xlab('Ano') +
