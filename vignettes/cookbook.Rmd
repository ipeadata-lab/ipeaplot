...
graf1 <- ggplot(data=base_graf1, aes(x=as.numeric(as.character(Ano)), y=Populacao)) +
  ...
  geom_text(aes(label = gsub("\\.", ",",round(Populacao,1))), hjust = 0.5, vjust = -0.5, size = 6, family = "Frutiger-LT-55-Roman")
...
graf3 <- ggplot(base_graf3, aes(x= as.character(pos), y=Gasto, fill=Pais,
  ...
  geom_text( position = position_stack(vjust = 0.5), color = 'white', size = 3, family = "Frutiger-LT-55-Roman") +
  geom_text(data = temp, aes(y = total ,label = paste0("U$ ",total)), vjust = -.5, size = 6, family = "Frutiger-LT-55-Roman") +
...
```{r eval=TRUE, message=FALSE, fig.align="center", fig.width=6, fig.height=6}
