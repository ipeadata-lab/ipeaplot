...
graf2 <- ggplot(base_graf2, aes(x=Ano, y=Populacao, group = 1))+
  ...
  theme_ipea(x_breaks = 16, x_text_angle = 90, include_ticks = F,expand_y_limit = T,
             include_x_text_title = F,include_y_text_title = F) +
  geom_rug(data = ref, aes(x = Ano+1),inherit.aes = FALSE,
           outside = TRUE, sides = "b", length = unit(2, "mm"), linewidth = 0.25) +
...
label <- (base_graf8_new %>% ungroup() %>% distinct(pais))$pais

graf8 <-
  ggplot() +
    geom_bar(data=base_graf8_new, aes(x=n-0.5, y=value,
    ...
    geom_line(data=base_graf8_aux, aes(x=n-0.5 ,y=`crescimento do PIB (ppc)_perct`, group = 1),
              color = 'gray') +
    scale_x_continuous(breaks = 1:max(base_graf8_new$n), labels = label) +
    scale_fill_ipea() +
    theme_ipea(legend.position = 'bottom', x_text_angle = 90, include_ticks = F,
               include_x_text_title = F,include_y_text_title = F, expand_x_limit = T) +
    geom_rug(data=base_graf8_new, aes(x = n - 1),
             outside = TRUE, sides = "b", length = unit(2, "mm"), linewidth = 0.25) +
    coord_cartesian( clip = "off")
