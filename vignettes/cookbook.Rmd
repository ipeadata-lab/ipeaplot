...
O [**ipeaplot**](https://github.com/ipea-datalab/ipeaplot) é um pacote computacional desenvolvido para facilitar a padronização de cores e formatação gráfica para submissões de textos para discussão (TD) e outras publicações do Instituto de Pesquisa Econômica Aplicada (IPEA). Este pacote fornece aos usuários ferramentas e funções que simplificam o processo de aplicação de esquemas de cores e estilos gráficos consistentes em seu trabalho, garantindo a conformidade com os requisitos e diretrizesdo Editorial do Ipea. Esta vinheta apresenta introdução rápida ao **ipeaplot**.
...
```{r eval=TRUE, message=FALSE, warning=FALSE, fig.align="center", fig.width=5, fig.height=5}
...
graf2 <- ggplot(base_graf2, aes(x='', y=percentual, fill=Assunto))+
  ...
  geom_text(aes(x = 1.8, label = paste0(percentual,"%")), position = position_stack(vjust = .5))
...
```{r eval=TRUE, message=FALSE, warning=FALSE, fig.align="center", fig.width=5, fig.height=6}
...
```{r eval=TRUE, message=FALSE, fig.align="center", fig.width=5, fig.height=5}
...
```{r eval=TRUE, message=FALSE, warning=FALSE, fig.align="center", fig.width=4, fig.height=6}
...
Gráfico 9 - Original
```{r, echo=FALSE, message=FALSE, warning=FALSE}
knitr::include_graphics(paste0(getwd(),"/prints_original/grafico9.png"))
```
...
graf9 <-
  ...
  geom_line(data=base_graf9_aux, aes(x=n ,y=`crescimento do PIB (ppc)_perct`, group = 1),
            color = 'gray') +
...
Gráfico 10 - Original
```{r, echo=FALSE, message=FALSE, warning=FALSE}
knitr::include_graphics(paste0(getwd(),"/prints_original/grafico10.png"))
```
